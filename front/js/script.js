...
(function () {
	...
	let
			...
			plugins = {
				...
				wow:                     $('.wow'),
				parallaxJs:              document.querySelectorAll('.parallax-scene-js'),
			};
	...
	$(function () {
		...
		// Parallax JS
		let screenLg = window.innerWidth;
		if ( plugins.parallaxJs.length && !isNoviBuilder && screenLg > 992 && !isMobile ) {
			for ( let i = 0; i < plugins.parallaxJs.length; i++ ) {
				let parallaxJS =  new Parallax( plugins.parallaxJs[i], {
					selector: '.layer',
				} );

				$window.on('resize', function () {
					screenLg = window.innerWidth;
					if ( screenLg < 992 ) {
						parallaxJS.disable();
						parallaxJS.layers.forEach( function ( item ) {
							item.style.position = 'relative';
							item.style.transform = 'none';
						} );
					} else if ( screenLg > 992 ) {
						parallaxJS.enable();
					}
				})
			}
